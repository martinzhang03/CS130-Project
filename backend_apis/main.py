...
async def register_user(user: UserEmail, db: Database = Depends(db.get_db)):
    try:
...
async def confirm_user(user: UserConfirmation, request: Request, db: Database = Depends(db.get_db)):
    try:
        ...
        return {"status": "success", "message": "code confirmed", "jwt_token": token}
...
async def login_user(user: UserLogin, request: Request, db: Database = Depends(db.get_db)):
    try:
